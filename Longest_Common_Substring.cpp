...
int main() 
{
	...
	for (int i=1; i<=n; i++)
		for (int j=1; j<=m; j++) {
			if(s1[i-1] == s2[j-1])
				dp[i][j] = 1 + dp[i-1][j-1]; //(add 1 to the result and remove the last character from both the strings and check the result for the smaller string.)
			else
				dp[i][j] = 0; //Since its substring and not subsequence, this killing spree ends here.
		}
